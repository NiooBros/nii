...
const OpenGameMenu = () => {
	try
	{
		...
		gm.discord(translateText("Explores the inventory"));
...
const getDrops = () => {
	try
	{
        ...
        if (DropsObject.length > 0) {
            mp.gui.emmit(`window.events.callEvent("cef.inventory.InitOtherData", 8, '${translateText("On Earth")}', '${JSON.stringify(DropsObject)}', ${DropsObject.length})`);
        } else {
