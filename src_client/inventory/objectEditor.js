...
gm.events.add('click', (x, y, upOrDown, leftOrRight, relativeX, relativeY, worldPosition, hitEntity) => {
	try
	{
		...
		if (leftOrRight == 'left' && global.localplayer.isInWater()) return mp.events.call('notify', 1, 9, translateText("You can not install objects here"), 3000);
		...
		//if (global.isInSafeZone) return mp.events.call('notify', 1, 9, translateText("Can not be placed in the green zone"), 3000);
...
gm.events.add('client.dropinfo.close', (isEnter = false) => {
	...
	// mp.events.call('notify', 1, 9, translateText("Can not be placed in the green zone"), 3000);
